...
# Calculate the Y position of the bottom line
bottom_line_y = screen_height - 1
...
def accumulate_snow():
    for flake in snowflakes:
        x, y, _, _, _ = flake
        if y >= bottom_line_y:
            # Calculate the column where the snowflake landed
            column = min(max(int(x), 0), screen_width - 1)
            # Add to snow accumulation at the corresponding column
            if len(snow_accumulation[0]) < 8:
                for row in range(8):
                    snow_accumulation[row][column] += 1
            # Stop the snowflake
            flake[1] = bottom_line_y
...
while running:
    ...
    for flake in snowflakes:
        ...
        if y <= bottom_line_y:
            ...
            if y > bottom_line_y or x >= screen_width or x < 0:
                x = random.randint(0, screen_width)
                y = 0
    ...
    for row, y_values in enumerate(snow_accumulation):
        for x, depth in enumerate(y_values):
            for _ in range(depth):
                pygame.draw.circle(screen, WHITE, (x, bottom_line_y - depth), 2)
